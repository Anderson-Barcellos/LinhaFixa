...
Notas: validação pendente no iPhone Pro Max (portrait + landscape, zoom natural): faixa colapsada legível, puçador, calibração com guia novo. Fallback mapeado se a faixa sheet ficar apertada em 390px: encurtar rótulos dos chips. Pós-revisão: D2 expandable cards na fila. Higiene 2026-07-04: checkpoint `f4c8ff7` commitou o pendente das sessões anteriores (D1 line-return + recall + smoke + PN1-3); worktree `quirky-euclid-a4dd5e` e branch removidos (conteúdo era rascunho obsoleto do design de 20s, já superado na main).
...
Rotacionado em 2026-07-04 (tarde) apos entrega do BUNDLE MD1 (gaveta mobile). Working tree LIMPA, tudo commitado ate `d3e60c1`; historico regularizado (checkpoint `f4c8ff7` absorveu D1/recall/smoke/PN1-3 que estavam deployados sem commit); worktree/branch quirky-euclid removidos.
...
- D2 — expandable cards (fila pos-gaveta).
- Fase 0 (capturas etiquetadas manuais) SO depois do VC4 fechar — senao os dados nascem contaminados.

Ler primeiro na retomada: esta secao; PACK Layout Mobile (topo); `.remember/now.md` (handoff 15:35).
Validacao padrao: `npm run test`, `npx tsc --noEmit`, `APP_BASE_PATH=/gaze npm run build`; deploy = `systemctl restart linhafixa.service` + `npm run smoke` (55 checks, cobre gaveta e calibracao nos 4 viewports). NAO usar vitest (41 fails falsos). Servico e `linhafixa.service` (nao "gaze"); env em `/etc/linhafixa.env`.
