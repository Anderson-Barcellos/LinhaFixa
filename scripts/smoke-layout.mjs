...
async function runViewport(browser, profile) {
  ...
  page.on('requestfailed', request => {
    ...
    // Navegação plena aborta preloads ociosos em andamento; abort não é asset quebrado.
  ...
  page.on('response', response => {
    ...
    if (expectation && response.ok()) {
      requiredAssetValidations.push((async () => {
        // finished() nunca resolve quando a navegação mata a request no meio
        // (gotcha do Playwright); teto de 15s e saída silenciosa — asset quebrado
        // de verdade continua barrado pelo flag requiredAssets e pelo MIME.
        const finishError = await Promise.race([
          response.finished(),
          new Promise(resolve => setTimeout(resolve, 15_000, new Error('finished() timeout')).unref()),
        ]);
